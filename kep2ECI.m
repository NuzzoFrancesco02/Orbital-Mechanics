function [r, v, t] = kep2ECI(t, a, e, i, OM, w, th0, mu)
...
% -------------------------------------------------------------------------
    ...
    if length(t) > 2
        ...
        theta = kepler_equation(t, e, a, mu, t(1), th0);  % Compute the true anomaly
        % Convert Keplerian elements to Cartesian coordinates (position and velocity)
        [r, v] = kep2car(a, e, i, OM, w, theta, mu);  % Convert to ECI
    ...
    elseif length(t) == 2
        ...
        theta_beg = kepler_equation(t(1), e, a, mu, t(1), th0);  % At t(1)
        theta_end = kepler_equation(t(2), e, a, mu, t(1), th0);  % At t(2)
        
        % Interpolate the true anomaly values between the start and end points
        th = linspace(theta_beg, theta_end, 1e4)';  % Linspace for smooth transition
        
        % Convert Keplerian elements to Cartesian coordinates over the interpolated range
        [r, v] = kep2car(a, e, i, OM, w, th, mu);  % Convert to ECI
        
        % Recalculate the time vector using inverse Kepler's equation
        t = kepler_inv_equation(e, a, mu, 0, th);  % Time corresponding to true anomaly
    end
